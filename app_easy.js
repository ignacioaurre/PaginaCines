...
moviesTitulo.push(movies[i].title + '\n');
}
let moviesAlfabeticas = moviesTitulo.sort()

function cantidadMasVotadas(){
	let masVotadas = 0;
	for (let i = 0; i < movies.length; i++){
...
function ratingPromedio (){
	...
	for (let i = 0; i < movies.length; i++){
		if (movies[i].vote_average >= 7){
			puntaje = puntaje + movies[i].vote_average;
			cantidad++;
		}
	}
	let resultado = puntaje/cantidad;
	return resultado.toFixed(2);
}

function peliculasRatingAlto(){
	let pelicula = "";
	for (let i = 0; i < movies.length; i ++){
		if (movies[i].vote_average >=7){
		pelicula += ('Título: ' + movies[i].title + '\n' +
					 'Puntaje: ' + movies[i].vote_average + '\n' +
					 'Reseña: ' + movies[i].overview + '\n' + '\n');
		}
	}
	return pelicula;
}
...
function listadoSalas(){
	...
	for (let i = 0; i < theaters.length; i ++){
		salas += ('Sala: ' + theaters[i].name + '\n' +
					 'Dirección: ' + theaters[i].address + '\n' +
					 'Descripción: ' + theaters[i].description + '\n' + '\n');
	}
	return salas;
}


function tituloReseña(){
	let pelicula = "";
	for (let i = 0; i < movies.length; i ++){
		pelicula += ('Título: ' + movies[i].title + '\n' +
					'Reseña: ' + movies[i].overview + '\n' + '\n');
...
function preguntasFrecuentes(){
	...
	for (let i = 0; i < faqs.length; i ++){
		preguntas += ('Pregunta: ' + faqs[i].faq_title + '\n' +
					  'Respuesta: ' + faqs[i].faq_answer + '\n' + '\n');
		}
	return preguntas;
}
	...
	switch (req.url) {
		...
		case '/':
			res.end('Bienvenidos a DH Movies el mejor sitio para encontrar las mejores  películas, incluso mucho mejor que Netflix, Cuevana y PopCorn​\n' + 
					'Hay un  total de ' + movies.lenght + ' en cartelera\n' +
					'Las películas en cartelera son: ' + moviesAlfabeticas + '\n' +
					'Recorda que podes visitar las siguientes secciones: \n' + 
					'1) En cartelera:  /en-cartelera\n' + 
					'2) Mas votadas:  /mas-votadas\n' + 
		...
		case '/mas-votadas':
			res.end('Más Votadas \n' +
					'Total de peliculas entre las mas votadas: ' + cantidadMasVotadas() + '\n\n' +
					'Rating promedio entre las mas votadas: ' + ratingPromedio() + '\n\n' +
					'Lista de películas: \n' + peliculasRatingAlto() );
			break;
		case '/sucursales':
